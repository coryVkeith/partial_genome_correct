...
REF_COUNT=0
for f in ${BETA_DIR}/${SMPLE}/NC_*.fasta
     do REF_COUNT=$(( REF_COUNT + 1 ))
     echo $f >> ${BETA_DIR}/${SMPLE}/REF_NAMES.txt
done
...
while IFS= read -r current_line; do
    REFS+=("$current_line")
done < ${BETA_DIR}/${SMPLE}/REF_NAMES.txt

###Combine partial fasta files into one multiple fasta
cat ${BETA_DIR}/${SMPLE}/*NODE* > ${BETA_DIR}/${SMPLE}/${SMPLE}_combined_partials.fasta


$MINIMAP -k 10 -w 5 -B 0 -ado ${REFS[0]} ${BETA_DIR}/${SMPLE}/${SMPLE}_combined_partials.fasta > ${BETA_DIR}/${SMPLE}/${SMPLE}_minimap.sam
     # Convert sam to bam
     samtools view -S -b ${BETA_DIR}/${SMPLE}/${SMPLE}_minimap.sam > ${BETA_DIR}/${SMPLE}/${SMPLE}_minimap.bam
     # Sort the alignment
     samtools sort ${BETA_DIR}/${SMPLE}/${SMPLE}_minimap.bam -o ${BETA_DIR}/${SMPLE}/${SMPLE}_minimap_sorted.bam
     # Get consensus fastq file
     samtools mpileup -B -uf ${REFS[0]} ${BETA_DIR}/${SMPLE}/${SMPLE}_minimap_sorted.bam | bcftools call -c | vcfutils.pl vcf2fq > ${BETA_DIR}/${SMPLE}/${SMPLE}_tmp_consensus.fastq
     ...
     echo $SEQ_DESC1 $seq_ref1 >> ${BETA_DIR}/${SMPLE}/seq_desc.txt
     cat ${BETA_DIR}/${SMPLE}/${SMPLE}_tmp_consensus.fastq | seqkit replace -p $seq_ref1 -r $SEQ_DESC1 > ${BETA_DIR}/${SMPLE}/${SMPLE}_consensus.fastq
 #    rm ${BAM1}_tmp_consensus.fastq
     # Convert .fastq to .fasta
     seqtk seq -a ${BETA_DIR}/${SMPLE}/${SMPLE}_consensus.fastq > ${BETA_DIR}/${SMPLE}/${SMPLE}_${seq_ref1}_partial_corrected.fasta


###bowtie2 build index of filtered references above
if [[ -f "$BETA_DIR/${SMPLE}/${SMPLE}_${seq_ref1}_partial_corrected.fasta" ]]; then
    Combined_partials="$BETA_DIR/${SMPLE}/${SMPLE}_${seq_ref1}_partial_corrected.fasta"
    $BOWTIE/bowtie2-build -f $Combined_partials $BETA_DIR/${SMPLE}/bowtie2/idx/${SMPLE}_idx
    else
...
if [[ -f "$BETA_DIR/${SMPLE}/bowtie2/idx/${SMPLE}_idx.1.bt2" ]]; then    
    $BOWTIE/bowtie2  -x $BETA_DIR/${SMPLE}/bowtie2/idx/"${SMPLE}_idx" -q -1 ${RAW}/$F1 -q -2 ${RAW}/$R1 -S $BETA_DIR/${SMPLE}/bowtie2/${SMPLE}.SAM
    else
...
if [[ -f "${BETA_DIR}/${SMPLE}/bowtie2/${SMPLE}.SAM" ]]; then
    samtools view -S -b ${BETA_DIR}/${SMPLE}/bowtie2/${SMPLE}.SAM > ${BETA_DIR}/${SMPLE}/bowtie2/${SMPLE}.bam
    samtools sort ${BETA_DIR}/${SMPLE}/bowtie2/${SMPLE}.bam  -o ${BETA_DIR}/${SMPLE}/bowtie2/${SMPLE}_sorted.bam
    samtools mpileup --max-depth 0 -uf $Combined_partials ${BETA_DIR}/${SMPLE}/bowtie2/${SMPLE}_sorted.bam | bcftools call -c | vcfutils.pl vcf2fq > ${BETA_DIR}/${SMPLE}/bowtie2/${SMPLE}.fastq
...
###                                                                                                                                   |
#_____________________________________________________________________________________________________________________________________|    
    cp ${BETA_DIR}/${SMPLE}/bowtie2/${SMPLE}_partial_corrected.fasta "${BETA_DIR}/combined_fasta"
    rm ${BETA_DIR}/${SMPLE}/bowtie2/${SMPLE}.SAM
    rm ${BETA_DIR}/${SMPLE}/bowtie2/${SMPLE}.bam
    else
...
if [[ -f "${BETA_DIR}/${SMPLE}/bowtie2/${SMPLE}_sorted.bam" ]]; then
        f="${BETA_DIR}/${SMPLE}/bowtie2/${SMPLE}_sorted.bam"
        bedtools genomecov -bg -ibam $f > ${BETA_DIR}/${SMPLE}/bowtie2/${SMPLE}.coverage
        sed -i -r 's/(\s+)?\S+//2' ${BETA_DIR}/${SMPLE}/bowtie2/${SMPLE}.coverage
    touch ${BETA_DIR}/${SMPLE}/bowtie2/coverage.txt
else
...
if [[ -f "${BETA_DIR}/${SMPLE}/bowtie2/${SMPLE}.coverage" ]]; then
    module load R
    Rscript $SCRIPT_DIR/cov_for_graph.R $BETA_DIR $SMPLE --save
    touch ${BETA_DIR}/${SMPLE}/bowtie2/svg.txt
else
    echo "Error in generating coverage graphs. No coverage information used to build graph" >> $ERRORS/${SMPLE}_error.log
fi

if [[ -f "${BETA_DIR}/${SMPLE}/bowtie2/svg.txt" ]]; then
    cp ${BETA_DIR}/${SMPLE}/bowtie2/*.svg ${BETA_DIR}/combined_cov
else
